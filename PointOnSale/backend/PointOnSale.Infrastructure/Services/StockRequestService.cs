    ...
    ) : IStockRequestService
{
    ...
    public async Task ApproveRequestAsync(int requestId, int approverScopeId, CancellationToken cancellationToken = default)
    {
        ...
        // Transactional Transfer: Inventory + Wallet (Moved from Fulfill to Approve as per requirement)
        ...
        try
        {
            ...
            {
                // 1. Check Inventory
                var balance = await inventoryRepository.GetBalanceAsync(request.ToScopeNodeId, item.ProductId, cancellationToken);
                var available = balance?.QtyOnHand ?? 0;
                if (available < item.Qty)
                    throw new InvalidOperationException($"Insufficient inventory for Product {item.ProductId}. Available: {available}, Requested: {item.Qty}");
            ...
            if (totalAmount > 0)
            {
                ...
                await walletService.ProcessTransferAsync(
                    ...
                    "StockRequest", 
                    request.Id.ToString(), 
                    "Stock Request Approval Payment", 
